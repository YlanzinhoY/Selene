...
Cada instalação abre uma transação antes de executar o primeiro processo upstream. O journal e os backups ficam em:
...
Downloads podem ser cancelados sem rollback porque ocorrem antes do snapshot. Depois que a mutação começa, a TUI bloqueia a saída normal até commit ou rollback. Se houver queda de energia ou encerramento forçado, a transação permanece `active` e pode ser recuperada com `selene rollback --yes`.
